...
class SIRM_deterministic_per_countr_date_1_ver_1():
    ...
    def solve(var, country_atribute,list_SIRM,list_inter, inter_dates):
        ...
        if gamma <5 and mort<5:
            solution = spint.odeint(SIRM_deterministic_per_countr_date_1_ver_1.SIRM_model, [S0, I0, R0, M0], t, 
    ...
    def SIRM_model(y, t, country_beta, gamma, mort, inter_param, inter_feature, date_inter, intercept, inter_dates):
        if t>= date_inter and t<= date_inter+14:
            beta = np.abs(country_beta + (t-date_inter)*(inter_param) + intercept)
        elif t>date_inter+14:
            beta = np.abs(country_beta + 14*(inter_param) + intercept)
        else:
    ...
    def fit(self, country_atribute, list_SIRM, list_inter, inter_dates, iter_max = 350, pop_size = 600, opt = 'meta'):
        ...
        def Obj_function(var, country_atribute, list_SIRM, list_inter, inter_dates):
            obj_fun = 0
            _, solution_index_list, _ = SIRM_deterministic_per_countr_date_1_ver_1.solve(var, country_atribute,list_SIRM,list_inter, inter_dates)
            y = list_SIRM.values
    ...
    def predict(self, list_SIRM, list_inter, inter_dates, country_atribute):
        ...
        self.solution_list, self.solution_index_list, self.list_SIRM = SIRM_deterministic_per_countr_date_1_ver_1.solve(self.res.x, country_atribute, list_SIRM, list_inter, inter_dates)
...
if __name__=="__main__":
    ...
    for i in range(country_atribute.shape[0]):
        ...
        objekat = SIRM_deterministic_per_countr_date_1_ver_1(name = list_inter_x.iloc[0,0])
