...
class SIRM_deterministic_T2():
    ...
    def solve(var, country_atribute,list_SIRM,list_inter, inter_dates):
        ...
        for i in range(country_atribute.shape[0]):
            ...
            inter_param_t = var[3*country_atribute.shape[1]:3*country_atribute.shape[1]+list_inter[0].iloc[:,2:].shape[1]]
            inter_param_t2 = var[3*country_atribute.shape[1]+list_inter[0].iloc[:,2:].shape[1]:-3]
            ...
            if gamma <10 and mort<10:
                solution = spint.odeint(SIRM_deterministic_T2.SIRM_model, [S0, I0, R0, M0], t, 
                        args = (country_beta, gamma, mort, inter_param_t, inter_param_t2, inter_feature, date_inter, intercept_beta, inter_dates[i,:]))
            else:
    ...
    @staticmethod 
    def SIRM_model(y, t, country_beta, gamma, mort, inter_param_t, inter_param_t2, inter_feature, date_inter, intercept, inter_dates):
        idx = SIRM_deterministic_T2.find_nearest(date_inter,t)
        beta = np.abs(country_beta + (inter_feature[idx,:]*(t-inter_dates)).dot(inter_param_t) + (inter_feature[idx,:]*(t-inter_dates)**2).dot(inter_param_t2) + intercept)
        S, I, R, M = y
        dS_dt = - beta*S*I
    ...
    def fit(self, country_atribute, list_SIRM, list_inter, inter_dates, iter_max = 350, pop_size = 600, opt = 'meta'):
        ...
        def Obj_function(var, country_atribute, list_SIRM, list_inter, inter_dates):
            obj_fun = 0
            _, solution_index_list, _ = SIRM_deterministic_T2.solve(var, country_atribute,list_SIRM,list_inter, inter_dates)
            for i in range(country_atribute.shape[0]):
                y = list_SIRM[i].values
        ...
        bnd = ((-1,1),)*(3*country_atribute.shape[1]) + ((-1,1),)*list_inter[0].iloc[:,2:].shape[1]*2 + ((0, 2),)*3
    ...
    def predict(self, list_SIRM, list_inter, inter_dates, country_atribute):
        ...
        self.solution_list, self.solution_index_list, self.list_SIRM = SIRM_deterministic_T2.solve(self.res.x, country_atribute, list_SIRM, list_inter, inter_dates)
